...
if (notEmpty(sRowId) || notEmpty(sAuditPhoneNumber)){	//若有指定id或門號，則查單一門號的所有資料，若未指定門號，則查所有門號的最基本資料
	//sSQL = "SELECT A.id, DATE_FORMAT(A.Create_Date,'%y-%m-%d %H:%i'), A.Account_Name, A.Bill_Type, A.Line_Channel_Name, A.Audit_Phone_Number, A.Send_Notification, DATE_FORMAT(A.Billing_Start_Date,'%y-%m-%d %H:%i'), DATE_FORMAT(A.Expiry_Date,'%y-%m-%d %H:%i'), A.Status, B.Google_User_Name, B.Google_Email, B.Contact_Phone, B.Contact_Address, B.Tax_ID_Number, B.Purchase_Quantity, B.Member_Quantity";
	sSQL = "SELECT A.id, DATE_FORMAT(A.Create_Date,'%y-%m-%d'), A.Account_Name, A.Bill_Type, A.Line_Channel_Name, A.Audit_Phone_Number, A.Send_Notification, DATE_FORMAT(A.Billing_Start_Date,'%y-%m-%d'), DATE_FORMAT(A.Expiry_Date,'%y-%m-%d'), A.Status, B.Google_User_Name, B.Google_Email, B.Contact_Phone, B.Contact_Address, B.Tax_ID_Number, B.Purchase_Quantity, B.Member_Quantity";
}else{
	sSQL = "SELECT A.id, DATE_FORMAT(A.Create_Date,'%y-%m-%d'), A.Account_Name, A.Bill_Type, A.Line_Channel_Name, A.Audit_Phone_Number, A.Status";
}
...
if (sResultCode.equals(gcResultCodeSuccess)){	//有資料
	...
	String[] fields2 = null;
	if (notEmpty(sRowId) || notEmpty(sAuditPhoneNumber)){	//若有指定id或門號，則查單一門號的所有資料，若未指定門號，則查所有門號的最基本資料
		fields2 = new String[]{"id", "Create_Date", "Account_Name", "Bill_Type", "Line_Channel_Name", "Audit_Phone_Number", "Send_Notification", "Billing_Start_Date", "Expiry_Date", "Status", "Google_User_Name", "Google_Email", "Contact_Phone", "Contact_Address", "Tax_ID_Number", "Purchase_Quantity", "Member_Quantity"};
	}else{
		fields2 = new String[]{"id", "Create_Date", "Account_Name", "Bill_Type", "Line_Channel_Name", "Audit_Phone_Number", "Status"};
	}
